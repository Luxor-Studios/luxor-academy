...
function ensureHelpOverlay(): HTMLElement {
  ...
  const rows = SHORTCUTS.map(
    (s) => `<tr><th scope="row"><kbd>${escapeHtml(s.keys)}</kbd></th><td>${escapeHtml(s.desc)}</td></tr>`,
  ).join("");
  overlay.innerHTML = `
  ...
  overlay.addEventListener("click", (ev) => {
    ...
    if (t.dataset && t.dataset.close === "true") closeHelp();
  });
...
function openHelp(): void {
  ...
  const btn = overlay.querySelector<HTMLButtonElement>("button[data-close]");
  btn?.focus();
}
...
function toggleHelp(): void {
  const overlay = document.getElementById("luxor-help-overlay");
  if (overlay && !overlay.hidden) closeHelp();
  else openHelp();
}
...
export function installKeyboardShortcuts(): Unsub {
