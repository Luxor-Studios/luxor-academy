...
|---|---|---|---|
| **Default** (codebase-to-course) | A codebase (local folder or GitHub URL) | A directory with modular HTML + `styles.css` + `main.js`, assembled into `index.html` | Multi-module interactive course teaching how a project works |
| **skill-page** | A Claude Code skill, slash command, or agent | A single self-contained `.html` file (≤ 80 KB, everything inlined) | Teaching/reference page for a specific skill or command |
| **plugin-page** | A Claude Code plugin directory (multiple skills + commands + agents under one `plugin.json`) | A single aggregated `.html` page (≤ 80 KB) covering all sub-artifacts | One-page dev docs for a whole plugin, with cross-skill worked examples |
| **batch-skill-page** | A list of skill names (comma-separated, newlines, or file ref) | A directory of N per-skill HTML files + an `index.html` navigator | Generate many skill pages at once using parallel agents (cap 5 concurrent) |
| **course-with-modules** | An explicit list of module specs (inline text or markdown file refs) | Same multi-file directory as default mode | You already know the curriculum and want to skip codebase analysis |
| **repo-explainer** | A GitHub URL or local repo path, optionally with `focus_areas` | A single `.html` explainer (≤ 80 KB) with per-focus-area deep dives | Lightweight reference page for a repo — lighter than a course, heavier than a skill-page |

**One-line examples:**

- **Default:** *"Turn this codebase into an interactive course."*
- **skill-page:** *"Make a single-page teaching doc for the `/hekat` skill, save it to `./hekat-page.html`."*
- **plugin-page:** *"Make a plugin page for the `superpowers` plugin, save to `./superpowers-page.html`."*
- **batch-skill-page:** *"Batch skill pages for hekat, mercurio, codebase-to-course — save to `./docs/skills/`."*
- **course-with-modules:** *"Make a course with these modules: 1) Intro to LLMs … 2) Prompting basics … 3) Tool use …"*
- **repo-explainer:** *"Make a github repo explainer page for https://github.com/fastapi/fastapi with focus areas: dependency injection, pydantic integration."*

All six modes share the same design system (warm cream palette, Bricolage Grotesque display face, rotating per-section accents, IntersectionObserver fade-in motion). See the per-mode reference files under `references/` for each mode's spec.
